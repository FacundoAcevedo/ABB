...
void random__initialize(){
	unsigned int seed = (unsigned int) time (NULL);
	srand(seed);
}
int random_ (size_t lim){
	return rand()%lim;
}
...
void vector_desordenar(char* claves[], int* valores[], size_t largo){
	...
	for (i=0; i<largo;i++){
		rnd = random_(largo);
		swap(&claves[i], &claves[rnd]);
		swap(&valores[i], &valores[rnd]);
	}
}
...
void prueba_abb_volumen(size_t largo) 
{
	...
	for (int a = 0; a <largo; a++){
		printf("clave: %s", claves[a]);
		puts("");
	}
	for (int e = 0; e<largo; e++){
		*valores[e] = e;
		ok = abb_guardar(abb, claves[e], valores[e]);
	}
	...
	printf("Cant de elementos: %d, largo: %zu \n\n", abb_cantidad(abb), largo);
	...
	/* Destruye el abb y crea uno nuevo que sÃÂ­ libera */
	abb_destruir(abb);
	abb = abb_crear(cmp, NULL);
	...
	/* Destruye el abb - deberÃÂ­a liberar los enteros */
	abb_destruir(abb);
...
//~ 
	...
	prueba_abb_volumen(10);
	//~ prueba_abb_volumen(100);
